        ...
        static drawOverlay(id, village, status) {
          ...
          if (el) {
            ...
            if (status === "OFF") {
              ...
              el.innerHTML = '<span style="position:absolute; top:1px; right:1px; font-size:12px; line-height:1; padding:1px 2px; border-radius:3px; background:rgba(0,0,255,0.75); border:1px solid #00a; text-shadow:0 0 2px #000;">\u{1F6E1}\uFE0F</span>';
            }
          }
        }
  ...
  var init_Scraper = __esm({
    "src/dashboard/Scraper.ts"() {
      ...
      Scraper = class {
        ...
        fetchPageHtmlWithRetry(url, retries = 3, delayMs = 1200) {
          return new Promise((resolve, reject) => {
        ...
        parsePlayerTable(html, mode, headersList, displayHeadersList) {
          const parser = new DOMParser();
          const doc = parser.parseFromString(html, "text/html");
          const candidateTables = Array.from(doc.querySelectorAll("table.vis.w100, table.vis"));
          ...
          for (let i = 0; i < candidateTables.length; i++) {
            ...
            const { indexToKey: indexToKey2 } = this.buildHeaderIndexMap(table, mode, headersList, displayHeadersList);
            const mappedColumns = Object.keys(indexToKey2).length;
            if (mappedColumns >= 2) {
              dataTable = table;
              selectedIndex = i;
              break;
        ...
        readData(mode, onProgress, onComplete) {
          ...
          const buildingDisplayHeaders = ["Edificio Principal", "Cuartel", "Cuadra", "Taller", "Corte", "Herrer\xEDa", "Plaza de reuniones", "Estatua", "Mercado", "Le\xF1ador", "Barrera", "Mina de hierro", "Granja", "Almac\xE9n", "Escondrijo", "Muralla"];
          let headersList = [];
          let displayHeadersList = [];
          let parsePrimaryHeaders = [];
          let parseBuildingHeaders = [];
          let parseBuildingDisplayHeaders = [];
          ...
          const loop = () => {
            ...
            this.fetchPageHtmlWithRetry(url).then((result) => {
              ...
              const buildingHtmlPromise = mode === "members_troops" ? this.fetchPageHtmlWithRetry(`https://${window.location.host}/game.php?screen=ally&mode=members_buildings&player_id=${currentPlayer.playerId}&page=${pageNumber}`) : Promise.resolve("");
              buildingHtmlPromise.then((buildingHtml) => {
                ...
                const buildingVillages = mode === "members_troops" ? this.parsePlayerTable(buildingHtml, "members_buildings", parseBuildingHeaders, parseBuildingDisplayHeaders) : [];
                const villageMap = {};
                ...
                Object.values(villageMap).forEach((villageData) => {
                  ...
                  const row = `${villageData.x}|${villageData.y},${playerName},${villageData.points},` + headersList.map((item) => item ? villageData.values[item] || "0" : "0").join(",") + "\n";
                  console.log(`[Scraper] Fila a\xF1adida al CSV:`, row.trim());
                  csvData += row;
                ...
                console.error(`[Scraper] Error al cargar p\xE1gina adicional para playerId=${currentPlayer.playerId}, p\xE1gina=${pageNumber}:`, error);
                currentIndex++;
                pageNumber = 1;
                setTimeout(loop, 1200);
              });
            }).catch((error) => {
              console.error(`[Scraper] Error AJAX para playerId=${currentPlayer.playerId}, p\xE1gina=${pageNumber}:`, error);
              currentIndex++;
              pageNumber = 1;
              setTimeout(loop, 1200);
            });
  ...
  var init_Dashboard = __esm({
    "src/dashboard/Dashboard.ts"() {
      ...
      Dashboard = class {
        ...
        showPlayerDashboard(result) {
          const playerOptions = result.players.map(
        ...
        `;
          ...
          setTimeout(() => {
            ...
            const playerSelect = document.getElementById("tw-dash-player-select");
            if (playerSelect) {
        ...
        renderPlayerDetails(playerId, result) {
